...
export const CategoriesPage: React.FC = () => {
  ...
  const openCreate = () => { setEditing(null); setForm({ name: '', vertical: 'shop', image: '', slug: '' }); setModal(true); };
  const openEdit = (c: Category) => { setEditing(c); setForm({ name: c.name, vertical: c.vertical, image: c.image || '', slug: c.slug || '' }); setModal(true); };
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        ...
        <button onClick={openCreate} className="flex items-center gap-2 px-4 py-2 bg-[#0F2C59] text-white rounded-xl text-sm font-semibold hover:bg-[#1a3d73]">
          ...
          <tbody className="divide-y divide-gray-50">
            {loading ? (
              ...
              <tr key={c.id} className="hover:bg-gray-50/50">
                ...
                <td className="px-5 py-4">
                  <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${c.isActive ? 'bg-emerald-50 text-emerald-700' : 'bg-gray-100 text-gray-500'}`}>{c.isActive ? 'Active' : 'Inactive'}</span>
                </td>
                <td className="px-5 py-4">
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <form onSubmit={save} className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl space-y-4">
            ...
            <div>
              ...
              <input value={form.image} onChange={e => setForm({ ...form, image: e.target.value })} placeholder="https://…" className="w-full px-3 py-2 border rounded-xl text-sm" />
            </div>
            <div className="flex gap-2 pt-1">
              ...
              <button type="submit" className="flex-1 py-2.5 bg-[#0F2C59] text-white rounded-xl text-sm font-semibold hover:bg-[#1a3d73]">{editing ? 'Save' : 'Create'}</button>
            </div>
          </form>
        </div>
      )}
