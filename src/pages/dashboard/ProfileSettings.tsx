      ...
      <div className="bg-white border border-brand-border rounded-card shadow-premium p-6">
        <div className="flex items-start gap-6 flex-wrap mb-6">
          <div className="flex flex-col items-center gap-2">
            ...
            <div className="flex items-center gap-1.5 text-[10px] font-bold text-white bg-slate-700 px-2.5 py-1 rounded-full">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="px-2 py-0.5 bg-emerald-50 text-emerald-700 rounded-full text-[10px] font-bold border border-emerald-100">Plus Member</span>
            </div>
            <p className="text-xs text-brand-slate">{profile.email}</p>
            <p className="text-[11px] text-brand-slate mt-1">Member since {new Date().toLocaleDateString('en-IN', { month: 'long', year: 'numeric' })}</p>
          </div>
        </div>

        <form onSubmit={submit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="flex flex-col gap-1">
            <label className="text-[10px] font-black uppercase text-brand-slate flex items-center gap-1"><User className="w-3 h-3" /> Full Name</label>
            <input className={fieldCls} value={name} onChange={(e) => setName(e.target.value)} required />
          </div>
          <div className="flex flex-col gap-1">
            <label className="text-[10px] font-black uppercase text-brand-slate flex items-center gap-1"><Mail className="w-3 h-3" /> Email Address</label>
            <input className={fieldCls} type="email" value={email} onChange={(e) => setEmail(e.target.value)} required />
          </div>
          <div className="flex flex-col gap-1">
            <label className="text-[10px] font-black uppercase text-brand-slate flex items-center gap-1"><Phone className="w-3 h-3" /> Mobile Number</label>
            <input className={fieldCls} value={phone} onChange={(e) => setPhone(e.target.value)} />
          </div>
          <div className="flex flex-col gap-1">
            <label className="text-[10px] font-black uppercase text-brand-slate flex items-center gap-1"><Calendar className="w-3 h-3" /> Date of Birth</label>
